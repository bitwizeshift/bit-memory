  ...
  noexcept
{
  ...
  // Count the number of set bits; if two bits are set, the alignment
  // is invalid
  do {
    if( alignment & 1u ) ++c;
...
inline std::size_t bit::memory::align_forward_offset( void* ptr,
  ...
  noexcept
{
  assert( is_valid_alignment(align) && "Alignment must be a power of 2");
...
inline void* bit::memory::align_forward( void* ptr,
  ...
  noexcept
{
  assert( is_valid_alignment(alignment) && "Alignment must be a power of 2");
